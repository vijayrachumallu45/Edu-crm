import React, { useState } from 'react';
import { useEduContext } from '../../context/EduContext';
import { FeeRecord } from '../../types';
import { Badge } from '../../components/common/Badge';
import { Modal } from '../../components/common/Modal';
import { ActionMenu, ActionMenuItem } from '../../components/common/ActionMenu';
import { StatCard } from '../../components/common/StatCard';
import {
  CreditCard,
  Plus,
  Search,
  CheckCircle2,
  DollarSign,
  TrendingUp,
  AlertTriangle,
  History
} from 'lucide-react';

export const FeesPage: React.FC = () => {
  const { feeRecords, recordFeePayment, userProfile } = useEduContext();

  const [searchQuery, setSearchQuery] = useState('');
  const [selectedFee, setSelectedFee] = useState<FeeRecord | null>(null);
  const [isPaymentModalOpen, setIsPaymentModalOpen] = useState(false);
  const [isHistoryModalOpen, setIsHistoryModalOpen] = useState(false);
  const [paymentError, setPaymentError] = useState('');

  const [paymentForm, setPaymentForm] = useState({
    amount: 50000,
    method: 'Bank Transfer',
    reference: ''
  });

  const totalBilled = feeRecords.reduce((acc, f) => acc + f.totalFee, 0);
  const totalCollected = feeRecords.reduce((acc, f) => acc + f.paid, 0);
  const totalPending = feeRecords.reduce((acc, f) => acc + f.pending, 0);
  const overdueCount = feeRecords.filter(f => f.status === 'Overdue').length;

  const handleOpenPaymentModal = (fee: FeeRecord) => {
    setSelectedFee(fee);
    setPaymentError('');
    setPaymentForm({
      amount: Math.min(50000, fee.pending),
      method: 'Bank Transfer',
      reference: `TXN-${Math.floor(100000 + Math.random() * 900000)}`
    });
    setIsPaymentModalOpen(true);
  };

  const handleRecordSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedFee) return;

    if (!Number.isFinite(paymentForm.amount) || paymentForm.amount <= 0) {
      setPaymentError('Enter a payment amount greater than zero.');
      return;
    }

    if (paymentForm.amount > selectedFee.pending) {
      setPaymentError(`Payment cannot exceed the outstanding balance of ${userProfile.currency}${selectedFee.pending.toLocaleString()}.`);
      return;
    }

    recordFeePayment(selectedFee.id, Number(paymentForm.amount), paymentForm.method, paymentForm.reference);
    setPaymentError('');
    setIsPaymentModalOpen(false);
  };

  const filteredFees = feeRecords.filter(f =>
    f.studentName.toLowerCase().includes(searchQuery.toLowerCase()) ||
    f.program.toLowerCase().includes(searchQuery.toLowerCase())
  );

  const getStatusBadgeVariant = (status: FeeRecord['status']) => {
    switch (status) {
      case 'Paid': return 'success';
      case 'Partially Paid': return 'warning';
      case 'Pending': return 'default';
      case 'Overdue': return 'danger';
    }
  };

  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      {/* Metric Cards Top Row */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard
          title="Total Fees Billed"
          value={`${userProfile.currency}${(totalBilled / 100000).toFixed(1)}L`}
          subtitle="Gross program billing"
          icon={CreditCard}
          iconBgColor="bg-indigo-50 text-indigo-600 dark:bg-indigo-950/60 dark:text-indigo-400"
        />
        <StatCard
          title="Total Collected"
          value={`${userProfile.currency}${(totalCollected / 100000).toFixed(1)}L`}
          subtitle="Realized revenue"
          icon={TrendingUp}
          change="+14%"
          changeType="positive"
          iconBgColor="bg-emerald-50 text-emerald-600 dark:bg-emerald-950/60 dark:text-emerald-400"
        />
        <StatCard
          title="Total Pending"
          value={`${userProfile.currency}${(totalPending / 100000).toFixed(1)}L`}
          subtitle="Outstanding receivables"
          icon={DollarSign}
          changeType="neutral"
          iconBgColor="bg-amber-50 text-amber-600 dark:bg-amber-950/60 dark:text-amber-400"
        />
        <StatCard
          title="Overdue Accounts"
          value={overdueCount}
          subtitle="Requires immediate follow-up"
          icon={AlertTriangle}
          changeType="negative"
          iconBgColor="bg-rose-50 text-rose-600 dark:bg-rose-950/60 dark:text-rose-400"
        />
      </div>

      {/* Header Bar */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="relative flex-1 sm:w-72">
          <Search className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" />
          <input
            type="text"
            placeholder="Search fee records..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className="w-full pl-9 pr-4 py-2 text-xs bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-slate-900 dark:text-white placeholder-slate-400 focus:outline-hidden"
          />
        </div>
      </div>

      {/* Main Table */}
      <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200/80 dark:border-slate-700/80 shadow-xs overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-slate-100 dark:border-slate-700/80 bg-slate-50/50 dark:bg-slate-900/40 text-[11px] font-bold text-slate-500 uppercase tracking-wider">
                <th className="py-3.5 px-4">Student</th>
                <th className="py-3.5 px-4">Program</th>
                <th className="py-3.5 px-4">Total Fee</th>
                <th className="py-3.5 px-4">Paid</th>
                <th className="py-3.5 px-4">Pending</th>
                <th className="py-3.5 px-4">Status</th>
                <th className="py-3.5 px-4 text-right">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100 dark:divide-slate-700/60 text-xs">
              {filteredFees.length === 0 ? (
                <tr>
                  <td colSpan={7} className="py-12 text-center text-slate-400">
                    <p className="font-semibold text-slate-700 dark:text-slate-300 text-sm">No fee records found</p>
                  </td>
                </tr>
              ) : (
                filteredFees.map(fee => {
                  const menuItems: ActionMenuItem[] = [
                    {
                      label: 'Record Payment',
                      icon: CreditCard,
                      variant: 'primary',
                      onClick: () => handleOpenPaymentModal(fee)
                    },
                    {
                      label: 'View Payment History',
                      icon: History,
                      onClick: () => {
                        setSelectedFee(fee);
                        setIsHistoryModalOpen(true);
                      }
                    }
                  ];

                  return (
                    <tr key={fee.id} className="hover:bg-slate-50/60 dark:hover:bg-slate-700/40 transition-colors">
                      <td className="py-3.5 px-4 font-semibold text-slate-900 dark:text-white">
                        {fee.studentName}
                      </td>
                      <td className="py-3.5 px-4 text-slate-600 dark:text-slate-300 max-w-xs truncate">
                        {fee.program}
                      </td>
                      <td className="py-3.5 px-4 font-semibold text-slate-900 dark:text-white">
                        {userProfile.currency}{fee.totalFee.toLocaleString()}
                      </td>
                      <td className="py-3.5 px-4 font-semibold text-emerald-600 dark:text-emerald-400">
                        {userProfile.currency}{fee.paid.toLocaleString()}
                      </td>
                      <td className="py-3.5 px-4 font-semibold text-rose-600 dark:text-rose-400">
                        {userProfile.currency}{fee.pending.toLocaleString()}
                      </td>
                      <td className="py-3.5 px-4">
                        <Badge variant={getStatusBadgeVariant(fee.status)}>
                          {fee.status}
                        </Badge>
                      </td>
                      <td className="py-3.5 px-4 text-right">
                        <ActionMenu items={menuItems} />
                      </td>
                    </tr>
                  );
                })
              )}
            </tbody>
          </table>
        </div>
      </div>

      {/* Record Payment Modal */}
      {selectedFee && (
        <Modal
          isOpen={isPaymentModalOpen}
          onClose={() => setIsPaymentModalOpen(false)}
          title={`Record Payment for ${selectedFee.studentName}`}
          subtitle={`Current Outstanding: ${userProfile.currency}${selectedFee.pending.toLocaleString()}`}
          maxWidth="md"
        >
          <form onSubmit={handleRecordSubmit} className="space-y-4">
            <div>
              <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Payment Amount ({userProfile.currency}) *</label>
              <input
                type="number"
                required
                min="1"
                step="1"
                max={selectedFee.pending}
                value={paymentForm.amount}
                onChange={(e) => {
                  setPaymentError('');
                  setPaymentForm({ ...paymentForm, amount: Number(e.target.value) });
                }}
                className="w-full px-3 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl text-xs text-slate-900 dark:text-white focus:outline-hidden"
              />
              {paymentError && (
                <p className="mt-1 text-xs font-medium text-rose-600 dark:text-rose-400" role="alert">
                  {paymentError}
                </p>
              )}
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Payment Method</label>
                <select
                  value={paymentForm.method}
                  onChange={(e) => setPaymentForm({ ...paymentForm, method: e.target.value })}
                  className="w-full px-3 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl text-xs text-slate-900 dark:text-white focus:outline-hidden cursor-pointer"
                >
                  <option value="Bank Transfer">Bank Transfer</option>
                  <option value="UPI / Online">UPI / Online</option>
                  <option value="Cheque">Cheque</option>
                  <option value="Cash">Cash Deposit</option>
                </select>
              </div>
              <div>
                <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Transaction Ref / Cheque No.</label>
                <input
                  type="text"
                  value={paymentForm.reference}
                  onChange={(e) => setPaymentForm({ ...paymentForm, reference: e.target.value })}
                  className="w-full px-3 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl text-xs text-slate-900 dark:text-white focus:outline-hidden"
                  placeholder="e.g. TXN991823"
                />
              </div>
            </div>

            <div className="flex justify-end gap-3 pt-3 border-t border-slate-100 dark:border-slate-700">
              <button
                type="button"
                onClick={() => setIsPaymentModalOpen(false)}
                className="px-4 py-2 text-xs font-semibold text-slate-600 dark:text-slate-300 hover:bg-slate-100 rounded-xl"
              >
                Cancel
              </button>
              <button
                type="submit"
                className="px-4 py-2 text-xs font-semibold text-white bg-indigo-600 hover:bg-indigo-500 rounded-xl"
              >
                Confirm Payment
              </button>
            </div>
          </form>
        </Modal>
      )}

      {/* Payment History Modal */}
      {selectedFee && (
        <Modal
          isOpen={isHistoryModalOpen}
          onClose={() => setIsHistoryModalOpen(false)}
          title={`Payment History - ${selectedFee.studentName}`}
          subtitle={`${selectedFee.program}`}
          maxWidth="md"
        >
          <div className="space-y-3 text-xs">
            {selectedFee.paymentHistory.length === 0 ? (
              <p className="text-slate-400 text-center py-4">No payment transaction records found.</p>
            ) : (
              selectedFee.paymentHistory.map(pay => (
                <div key={pay.id} className="p-3 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-700 flex justify-between items-center">
                  <div>
                    <p className="font-bold text-emerald-600 dark:text-emerald-400 text-sm">+{userProfile.currency}{pay.amount.toLocaleString()}</p>
                    <p className="text-[10px] text-slate-400 mt-0.5">{pay.method} • Ref: {pay.reference}</p>
                  </div>
                  <span className="text-[11px] text-slate-500 font-mono">{pay.date}</span>
                </div>
              ))
            )}
          </div>
        </Modal>
      )}
    </div>
  );
};
// Enhanced fee collection reports
